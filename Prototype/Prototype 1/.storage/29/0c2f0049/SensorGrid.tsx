...
export default function SensorGrid({ onCharacterSelect, isDemoMode = false }: SensorGridProps) {
  ...
  const getSensorButtonClass = (character: string) => {
    const baseClass = "h-12 w-12 md:h-14 md:w-14 text-lg font-bold rounded-2xl transition-all duration-200 border-2";
    const isActive = activeCharacter === character;
    const isRecent = isRecentlyActivated(character);
    
    if (isActive) {
      return `${baseClass} bg-blue-600 text-white border-blue-600 scale-110 shadow-lg`;
    } else if (isRecent) {
      ...
      return `${baseClass} bg-slate-100 text-slate-500 border-slate-200 cursor-not-allowed`;
    }
  };
              ...
              <div key={rowIndex} className="flex justify-center gap-2 md:gap-3">
                {row.map((letter) => (
                  <Button
                    ...
                    disabled={!isDemoMode}
                    aria-label={`Letter ${letter} sensor${isDemoMode ? ' - click to test' : ''}`}
          ...
          <div className="flex justify-center flex-wrap gap-2 md:gap-3">
            {numbers.map((number) => (
              <Button
                ...
                disabled={!isDemoMode}
                aria-label={`Number ${number} sensor${isDemoMode ? ' - click to test' : ''}`}
        ...
        <CardContent className="pt-6">
          <div className="text-center">
            ...
            {isDemoMode ? (
              ...
              <p className="text-sm text-blue-700">
                Connect your BRAILLEAR hardware device to see real-time sensor activation. 
                Touch any sensor on the physical device to trigger audio feedback.
              </p>
            )}
          </div>
        </CardContent>
